...
def extract_features(
    ...
    pulse.iterate.iterate(
        forward_problem, (lvp, rvp), target_pressure, initial_number_of_steps=20
    )
    pulse.iterate.iterate(
        forward_problem, active_control, target_activation, initial_number_of_steps=150
    )
    u, p = forward_problem.state.split(deepcopy=True)

    # Simulated volume
    simulated_lv_volume = pulse.dolfin_utils.get_cavity_volume(
        geometry, chamber="lv", u=u
    )
    simulated_rv_volume = pulse.dolfin_utils.get_cavity_volume(
        geometry, chamber="rv", u=u
    )
    ...
    dx_lv = df.Measure(
        "dx", domain=geometry.mesh, subdomain_data=geometry.cfun, subdomain_id=1
    )
    dx_rv = df.Measure(
        "dx", domain=geometry.mesh, subdomain_data=geometry.cfun, subdomain_id=2
    )
    ...
    return (
        ...
        simulated_rv_volume,
        cauchy_fiber,
        cauchy_circumferential,
        cauchy_longitudinal,
        cauchy_radial,
        green_fiber,
        green_circumferential,
        green_longitudinal,
        green_radial,
    )
...
def compute_time_varying_elastance(
    ...
    forward_problem = create_forward_problem(
        geometry, material, p_endo_LV=lvp, p_endo_RV=rvp
    )
    ...
    E_t_dict = {
        "TVE": []
    }  # Dictionary to store the time-varying elastance (TVE) values
    ...
    for i in range(num_points):
        ...
        pulse.iterate.iterate(
            forward_problem,
            (lvp, rvp),
            (pv_data["LVP"][i], pv_data["RVP"][i]),
            initial_number_of_steps=20,
        )
        pulse.iterate.iterate(
            forward_problem,
            active_control,
            (LV_active_data[i], RV_active_data[i]),
        ...
        new_RV_cav_vol = pulse.dolfin_utils.get_cavity_volume(
            geometry, chamber="rv", u=u
        )
        logger.info(
...
def postprocess_simulation(
    ...
    LVV_ = [
        pulse.dolfin_utils.get_cavity_volume(unloaded_geometry, chamber="lv")
    ] + pv_data["LVV"]
    RVP_ = [0.0] + pv_data["RVP"]
    RVV_ = [
        pulse.dolfin_utils.get_cavity_volume(unloaded_geometry, chamber="rv")
    ] + pv_data["RVV"]
    ...
    for i, (lvp_, rvp_, lv_active_, rv_active_) in enumerate(
        zip(LVP_, RVP_, LV_active_data, RV_active_data)
    ):
        logger.info(
    ...
    make_spatial_plots(
        patient, unloaded_geometry, res, features, outdir="spatial_plots"
    )
