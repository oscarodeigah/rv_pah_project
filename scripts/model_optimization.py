...
def passive_optimization(
    ...
    def objective(x):
        ...
        for target_ps, target_vs in zip(pressure_data[:no_passive_filling_points], volume_data[:no_passive_filling_points]):
            pulse.iterate.iterate(forward_problem, (lvp,rvp), target_ps, initial_number_of_steps=20)
    ...
    start = datetime.now()
    res = minimize(objective, x0=x0, bounds=bnds, options={'disp': 5}, callback=print_control)
    print(res)
...
def active_optimization(
    ...
    assert pressure_data.shape == volume_data.shape, "Pressure and volume arrays are of different sizes"
    assert len(LV_active_data) == pressure_data.shape[0], "LV active data and pressure data should have the same length"

    material = define_material(
        geometry=unloaded_geometry,
        matparam_a=matparam_a,
    ...
    RV_active_values = [0.0] * no_passive_filling_points  # Fill RV_active_values list with zeros for the passive filling points.
    x0 = np.array([0.0])
    overall_start = datetime.now()
    for i, (target_ps, target_vs, target_LV_active) in enumerate(zip(pressure_data[no_passive_filling_points:], volume_data[no_passive_filling_points:], LV_active_data[no_passive_filling_points:])):
        logger.info(f"\nOptimizing pressure step, pressures = {target_ps}, volumes = {target_vs}")
        logger.info(f"\nLV active parameter {target_LV_active}")
        pulse.iterate.iterate(forward_problem, (lvp,rvp), target_ps, initial_number_of_steps=20)

        def objective(x):
            pulse.iterate.iterate(forward_problem,
                                  active_control,
        ...
        def jac(x,eps=1e-4):
            u, _ = forward_problem.state.split(deepcopy=True)
            LVV0 = pulse.dolfin_utils.get_cavity_volume(unloaded_geometry, chamber='lv', u=u)
            RVV0 = pulse.dolfin_utils.get_cavity_volume(unloaded_geometry, chamber='rv', u=u)
            _,RVV_obs = target_vs
            active_control.vector()[:] += eps
            _,RVV1 = forward_problem.solve_volumes()
            active_control.vector()[:] -= eps
            j = [2*((RVV0-RVV_obs)/RVV_obs)*(1/RVV_obs)*((RVV1-RVV0)/eps)]
            logger.info(f"\nComputed jacobian: {j}")
            return j
